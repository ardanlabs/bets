...
// addressRegex is the regular expression used to determine if a string is an address.
...
func init() {
	...
	// addressRegexString is the regular expression string used to compile into a regexp.
	const addressRegexString = "^0x[0-9a-fA-F]{40}$"
	addressRegex = regexp.MustCompile(addressRegexString)
}
...
// CheckAddress validates that the string is an address.
